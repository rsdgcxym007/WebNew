    ...
    <v-card elevation="8" class="mx-4 pa-4">
      ...
      <v-form ref="form" v-model="valid" lazy-validation class="px-4">
        ...
        <v-col cols="12" class="px-0" style="text-align: end">
          <v-btn :disabled="isReset" color="warning" @click="fetchData"
            >คืนค่า</v-btn
          >
          <v-btn color="success" @click="request()">ยืนยัน</v-btn>
        </v-col>
        <v-col>
          <v-select
            v-model="status"
          ...
          ></v-select>
          <div v-if="status === 'ef9e2e70-d55b-4250-8967-965b7cb0cbc7'">
            <v-row class="px-4">
              ...
              <v-col cols="12" style="text-align: end">
                <v-btn color="error" class="mr-4" @click="black">
                  ย้อนกลับ</v-btn
                >
                <v-btn :disabled="!valid2" color="success" @click="uploadImage">
                  บันทึกข้อมูล
                </v-btn>
              </v-col>
            </v-row>
...
export default {
  ...
  computed: {
    isReset() {
      return this.taskData.remark === this.$store.state.taskInfo.remark
    },
  },
}
</script>
